...
def pathModel = 'D:\\QuPath\\QuPath projects\\BIF\\yg.bif\\Script\\XC\\HE_1ch_1'
...
//        .normalizePercentilesGlobal(0.1, 99.8, 10) // Convenience global percentile normalization. arguments are percentileMin, percentileMax, dowsample.
        ...
        .tileSize(1500)                // If your GPU can take it, make larger tiles to process fewer of them. Useful for Omnipose
...
//        .excludeEdges()                // Clears objects toutching the edge of the image (Not of the QuPath ROI)
        .setOverlap(350)                // Overlap between tiles (in pixels) that the QuPath Cellpose Extension will extract. Defaults to 2x the diameter or 60 px if the diameter is set to 0
...
//        .useGPU()
//        .addParameter("exclude_on_edges") // Not GOOD if use cellExpansion
        .build()
...
QP.detectionToAnnotationDistancesSigned(true)
def max_D = 'Nucleus: Max diameter µm'
def distance = 'Signed distance to annotation with Training µm'
def toDelete = getDetectionObjects().findAll {(measurement(it, max_D)**2) >= ((measurement(it, distance)*2)**2) }
removeObjects(toDelete, true)

def distance1 = 'Signed distance to annotation with Validation µm'
def toDelete1 = getDetectionObjects().findAll {(measurement(it, max_D)**2) >= ((measurement(it, distance1)*2)**2) }
removeObjects(toDelete1, true)
logger.info('Remove nuclei Done!')
