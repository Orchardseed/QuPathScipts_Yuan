...
Hthreshold = 0.03 //Nuclei in Hematoxylin
...
boolean parallelizeAnnotations = true
...
def annotations = QP.getTMACoreList().findAll({!it.isMissing()})
if (annotations.isEmpty()) {
    println "Please select some objects!"
    return
}
else print("Calculating H-score for "+annotations.size()+" annotations...")
...
for (def annotation in annotations) {
    ...
    def pixhscore = (area1 * 1 + area2 * 2 + area3 * 3)/(areaDenom)*100 //calculate H score
    annotation.measurements['Pixelwise H-score'] = pixhscore
    print(""+annotation+": H-score = "+pixhscore)
}
