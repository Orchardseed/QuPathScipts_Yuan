...
def cellpose = Cellpose2D.builder(pathModel)
        ...
        .preprocess(new AddChannelsOp(scaling),
        )
...
class AddChannelsOp implements ImageOp {
    ...
    public Mat apply(Mat input) {
        ...
        if (channels.size() == 1)
            return input
        if (this.scaling.size() < channels.size()) {
            this.logger.error('WARNING! not enough channels defined for scaling. Skipping scaling.')
            return 0
        }
        this.logger.info('Scaling channels with '+scaling.toList().subList(0,channels.size()).inspect())
        def scaled = []
        for(int i = 0; i < channels.size(); i++)
            scaled.add(opencv_core.multiply(channels[i], this.scaling[i]))
        def sum = opencv_core.add(scaled[0], scaled[1])
        for (int i = 2; i < channels.size(); i++)
            sum = opencv_core.add(sum, scaled[i])
        return sum.asMat()
    }
}
