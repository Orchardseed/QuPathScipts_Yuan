...
imagelist.forEach({
    go = 0
    name = it.imageName.substring(0, it.imageName.length()-2).replace(' ', '_').replace('-','_')
    offsetdata.forEach({
        if (it.getname() == name){
            print("found "+name+" in offsets")
            top = it.gettop() // Change it to 0.0 when use mrxs file
            left = it.getleft() // Change it to 0.0 when use mrxs file
            go += 1
        }
    })
